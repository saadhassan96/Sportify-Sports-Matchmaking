      ...
      return new Promise((resolve, reject) => {
        task.on(
          ...
          () => {
            ...
            let ref = firebaseConf.database().ref('assets')
            ref.push({
              'URL': downloadURL,
              //'thumb': _imageData['thumb'],
              'name': name,
              //'coords': _imageData['coords'],
              'owner': firebaseConf.auth().currentUser && firebaseConf.auth().currentUser.uid,
    ...
    return (
      <View style={styles.mainContainer}>
        ...
        <View style={styles.contentContainer}>
          ...
          {
            this.state.image &&
            <Image source={{ uri: this.state.image }} style={{top: 10, width: 100, height: 100 }} />
          }
